...
**/
	...
	tBTNodePtr ptmp = (*RootPtr);
	tBTNodePtr pDest = NULL; // Posledni nalezeny uzel pro vkladani noveho prvku
	
	// Cast s hledanim pozice pro vlozeni:
	if( ptmp != NULL ) {
		do {
			pDest = ptmp;
			if( ptmp->Cont < Content ) {
				ptmp = ptmp->RPtr;
			}
			else if( ptmp->Cont > Content ) {
				ptmp = ptmp->LPtr;
			}
			else { // Pokud je hodnota nalezena, tak se nic nedela a jen se skonci
				return;
			}
	...
	tBTNodePtr pnew = malloc(sizeof(struct tBTNode));
	if( pnew == NULL ) {
		return;
	}
	pnew->Cont = Content;
	pnew->LPtr = pnew->RPtr = NULL;
	if( pDest == NULL ) {
		(*RootPtr) = pnew; // Strom byl prazdny
		return;
	}
	if( pDest->Cont < Content ) {
		pDest->RPtr = pnew;
	}
	else {
		pDest->LPtr = pnew;
...
**/
	...
	tBTNodePtr ptmp = RootPtr;
	Leftmost_Preorder(ptmp, &stack);

	while( !SEmptyP(&stack) ) {
		ptmp = STopPopP(&stack);
		Leftmost_Preorder(ptmp->RPtr, &stack);
	}
...
**/
	...
	tBTNodePtr ptmp = RootPtr;
	Leftmost_Inorder(ptmp, &stack);

	while( !SEmptyP(&stack) ) {
		ptmp = STopPopP(&stack);
		BTWorkOut(ptmp);
		Leftmost_Inorder(ptmp->RPtr, &stack);
	}
...
**/
	...
	Leftmost_Postorder(ptmp, &stack, &stackB);
	while( !SEmptyP(&stack) ) {
		ptmp = STopPopP(&stack);
		SPushP(&stack, ptmp);
		if( STopPopB(&stackB) ) { // Zpracuju prichod z leva
			SPushB(&stackB, FALSE); // Nastavim pruchod doprava
			Leftmost_Postorder(ptmp->RPtr, &stack, &stackB);
		}
		else {
			STopPopP(&stack);
			BTWorkOut(ptmp);
...
**/
	...
	do {
		if( (*RootPtr) == NULL ) {
			...
			if( (*RootPtr)->RPtr != NULL ) // Pokud vpravo neco je, tak ulozime
