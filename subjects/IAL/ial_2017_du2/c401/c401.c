...
**/
	...
	// Pokud se program dostal sem, tak nebyl nalezen klic
	return FALSE;
...
**/
	...
	if( (*RootPtr) == NULL ) {
		tBSTNodePtr ptmp = malloc(sizeof(struct tBSTNode));
		if(ptmp == NULL) {
			return;
		}
		ptmp->Key = K;
		ptmp->BSTNodeCont = Content;
		ptmp->LPtr = NULL;
		ptmp->RPtr = NULL;
		(*RootPtr) = ptmp;
	}
...
**/
	...
	if( (*RootPtr)->RPtr == NULL ) { // Nalezen nejpravejsi
		PtrReplaced->Key = (*RootPtr)->Key;
		PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;
		// Zaloha abych nestratil uzel
...
**/
	...
	else { // Nalezeno a ted je cas na mazani
		tBSTNodePtr ptmp = (*RootPtr);

		if(ptmp->LPtr == NULL) {
			(*RootPtr) = ptmp->RPtr;
			free(ptmp);
		}
		else if(ptmp->RPtr == NULL) {
			(*RootPtr) = ptmp->LPtr;
			free(ptmp);
		}
		else {
