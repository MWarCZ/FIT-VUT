...
** MV: Ehm v tomto souboru není potřeba komentovat. Jedná se zde jen o primitivní úkony.
**
...
*/
    if(s == NULL){ stackError(SERR_INIT); return; }
    s->top=-1;
}
...
** funkci stackEmpty.
*/
/* Toto funkce má nevhodně navržený prototyp. Nevidím důvod, proč by znak 
 * nemohl být vrácen rovnou funkcí. char stackTop(const tStack* s)
 * Poté by funkce šla lépe vzužít v cyklech. např.:
 * while(c = stackTop(s)){ ; }
 * */
     if(stackEmpty(s)){ stackError(SERR_TOP); return; }
     *c = s->arr[s->top];
}
...
** funkci stackFull.
*/
    if(stackFull(s)){ stackError(SERR_PUSH); return; }
    s->arr[++(s->top)]=c;
}
