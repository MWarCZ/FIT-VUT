...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
	...
	do {
		if(!stackEmpty(s)){
			stackTop(s,&c); // Špatný návrh prototypu funkce stackTop, proto se musí o znak žádat mimo podmínku cyklu.
			if(c!='(') // Pokud znak je operace, tak je přidána do výrazu.
				postExpr[(*postLen)++] = c;
			stackPop(s);
		}
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
	...
	else if(stackEmpty(s) || (c == '(') ){
		stackPush(s, c);
	}
	else {
		char sc;
		int priorityStack, priorityInput;
		do {
			stackTop(s, &sc); // Žádání externě o znak v zásobníku.
			// Vypočet priorit operandů. Bohužel nemůžeme použít vlastní makra, takže to vypadá tako. ;(
			priorityStack = (sc == '(')?0:((sc == '+')||(sc == '-'))?1:((sc=='*')||(sc=='/'))?2:3;
			priorityInput = ((c == '+')||(c == '-'))?1:((c=='*')||(c=='/'))?2:3;
			if(priorityInput <= priorityStack) {
				postExpr[(*postLen)++]=sc; // Přidáni do operace na konec výrazu a následné inkrementování délky výrazu
				stackPop(s);
			}
		}while(!stackEmpty(s) && priorityInput <= priorityStack);
		stackPush(s, c);
	}
}
...
char* infix2postfix (const char* infExpr) {
	...
	char* pNow = (char*)infExpr; // Ukazatel na aktuálné zpracovávaný znak.
	while(*pNow != '\0') {
		// Přidávání povolených znaků do výrazu.
		if( (*pNow >= 'a' && *pNow <='z')||(*pNow >= 'A' && *pNow <= 'Z')||(*pNow >= '0' && *pNow <='9') ){
			postExpr[postLen++]=*pNow;	
		}
		// Zakončení výrazu a vyprázdnění zbytku zásobníku.
		else if(*pNow == '=') {
			while(!stackEmpty(&s)){
				untilLeftPar(&s,postExpr,&postLen);
			}
		...
		// Ostatní znaky by měly být operace.
		else {
			doOperation(&s, *pNow, postExpr, &postLen);
		}
		pNow++; // Posunutí ukazatele na další znak.
	}
