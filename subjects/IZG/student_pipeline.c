...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  return (void const*) ( (size_t)head->buffer + (size_t)head->offset + (size_t)head->stride * (size_t)gl_VertexID );
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  VertexIndex vertexID = gpu_computeGLVertexID(
    puller->indices, 
  ...
  const GPUVertexPullerHead *pullerHead = NULL;

  for (int i = 0; i < MAX_ATTRIBUTES; i++) {
    pullerHead = &puller->heads[i];
    if (pullerHead->enabled) {
...
///  - GPUVertexShaderInput()
  ...
  VertexShaderInvocation offset;
  GPUVertexShaderOutput *vertex;

  for (int i = 0; i < nofPrimitiveVertices; i++) {
    offset = baseVertexShaderInvocation + i;
    gpu_runVertexPuller(
      &output, 
      puller, 
      offset
    ...
    input.gl_VertexID = gpu_computeGLVertexID(
      puller->indices, 
      offset
      );
